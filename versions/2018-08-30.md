...
### Globals
Globals是BSAM模板中的一个部分，用于定义所有CFC函数的通用属性。 所有`CFC:: Function`资源将继承此处定义的属性。

BSAM模板中的资源往往具有共享配置，例如运行时，内存，VPC设置，环境变量，CORS等。不需要在每个资源中复制此信息，可以在Globals部分中编写一次，并让所有Resource继承它。目前在Globals处可定义的属性只包括`CFC:: Function`的属性。
...
### Resource
 - [CFC::Function](#awsserverlessfunction)
